...
export function copyIPToClipboard(serverData) {
  const IP = formatMinecraftServerAddress(serverData.ip, serverData.port);
  navigator.clipboard
    .writeText(IP)
    .then(() => {
      // Blurred overlay
      const copySuccessOverlay = document.createElement('div');
      copySuccessOverlay.classList.add('copy-success-overlay');
      copySuccessOverlay.addEventListener('click', closeCopySuccessBox);
...
export function closeCopySuccessBox() {
  const copySuccessOverlay = document.querySelector('.copy-success-overlay');
  if (copySuccessOverlay) {
    document.body.removeChild(copySuccessOverlay);
  }

  const copySuccessBox = document.querySelector('.copy-success-box');
  if (copySuccessBox) {
    copySuccessBox.classList.add('hidden');
  }
